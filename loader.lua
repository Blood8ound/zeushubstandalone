...
local MAIN_URL = "https://raw.githubusercontent.com/Blood8ound/zeushubstandalone/main/zeushub"
...
local function setStage(nextStage, nextDetail)
    stage = nextStage
...
connection = RunService.RenderStepped:Connect(function()
    ...
    local wave = (math.sin(elapsed * 3.2) + 1) * 0.5
    local fillWidth = 70 + math.floor(wave * 270)
...
task.spawn(function()
    ...
    setStage("Compiling ZeusHub", tostring(#source) .. " bytes received")
    local chunk, compileError = loadstring(source)
    if not chunk then
        setStage("Compile failed", shorten(compileError, 54))
    ...
    setStage("Starting ZeusHub", "Initializing bundled UI")
    task.wait(0.08)
    local okRun, runError = pcall(chunk)
    if not okRun then
        setStage("Startup failed", shorten(runError, 54))
    ...
    setStage("Ready", "Press H to open the menu")
    task.wait(0.65)
    Loader.Stop()
end)
